# accounts/views.py (COMPLETE FIXED VERSION FOR MOBILE)
...
from students.forms import StudentProfileForm  # You may need to create this form
from employers.models import EmployerProfile
from employers.forms import EmployerProfileForm  # You may need to create this form
...
def user_login(request):
    """Fixed login view that works consistently across desktop and mobile"""
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            if not user.is_active:
                if request.content_type == 'application/json':
                    return JsonResponse({'error': 'Account is inactive. Please contact admin.'}, status=403)
            ...
            if not next_url:
                if user.role == 'student':
                    if not hasattr(user, 'student_profile'):
                        next_url = 'create_student_profile'
                    else:
                        next_url = 'student_dashboard'
                elif user.role == 'employer':
                    if not hasattr(user, 'employer_profile'):
                        next_url = 'create_employer_profile'
                    else:
                        next_url = 'employer_profile'
                else:
                    next_url = 'dashboard'
...
def dashboard(request):
    # Redirect users to their role-specific dashboards or profile creation
        ...
        if form.is_valid():
            try:
                ...
                if otp_success:
                    ...
                    messages.warning(
                        request,
                        f'Account created, but the verification code could not be sent. '
                        f'Admin has been notified. You can enter the code here after admin resends it.'
                    )
                    return redirect('verify_otp', user.id)
            
            except IntegrityError:
                messages.error(request, 'This email is already registered. Please log in or use another email.')
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                if otp_success:
                    messages.success(request, 'Employer account created. A verification code has been sent to your email.')
                    return redirect('verify_otp', user.id)
                else:
                    messages.warning(
                        request,
                        f'Employer account created, but the verification code could not be sent. '
                        f'Admin has been notified. You can enter the code here after admin resends it.'
                    )
                    return redirect('verify_otp', user.id)
...
def verify_otp(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            otp_has_expired = (user.otp_created_at and timezone.now() > user.otp_created_at + timedelta(minutes=15))
            
            if otp_has_expired:
                messages.error(request, 'This code has expired. Please ask the admin to send a new verification code.')
                ...
                messages.success(request, 'Verification successful! Please complete your profile.')
...
def create_student_profile(request):
    """Create student profile after verification"""
...
def create_employer_profile(request):
    """Create employer profile after verification"""
    # Check if profile already exists
    ...
    return render(request, 'accounts/create_employer_profile.html', {
        'form': form,
        'user': request.user
    })
...
def send_user_verification_code(request, user_id, channel='email'):
    ...
    user = get_object_or_404(User, id=user_id, role__in=['student', 'employer'], is_approved=False)
    
    if user.is_email_verified:
        messages.info(request, f'{user.username} is already verified and can now be approved.')
        return redirect('dashboard')
    ...
    if successful_deliveries == 0:
        ...
        user.otp_created_at = None
        user.save()
        messages.error(request, f'No verification code was delivered to {user.username}. {result_message}')
        return redirect('dashboard')
    
    if delivery_failed:
        messages.warning(request, f'Verification code process completed for {user.username}, but one channel failed. {result_message}')
    else:
        messages.success(request, f'Verification code sent to {user.username}. {result_message}')
...
def approve_user(request, user_id):
    ...
    if not user.is_email_verified:
        messages.error(request, 'This user must enter the admin-generated verification code. Successful verification activates the account automatically.')
        return redirect('dashboard')
    ...
    create_admin_notification(f'{role_label} account approved by {request.user.username}: {user.username} ({user.email}).')
    ...
    if not user_email_success or not admin_email_success:
        messages.warning(request, f'{user.username} approved successfully, but one email notification failed. User email: {user_email_message} Admin email: {admin_email_message}')
        return redirect('dashboard')
    
    messages.success(request, f'{user.username} approved successfully. User and admin notifications sent.')
    return redirect('dashboard')
...
def reject_user(request, user_id):
    ...
    user.delete()
    messages.warning(request, f'{username} has been rejected and removed. Email notification sent.')
    return redirect('dashboard')
